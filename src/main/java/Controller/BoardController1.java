...
public class BoardController1 { //김종인 작성
	...
	private static final int postCntPerPage = 20; // 한 페이지에 표시할 게시글 수
	private static final int pageCntPerPage = 10; // 한번에 표시할 페이지 개수
	...
	public ModelAndView basicBoard(
		...
		mv.setViewName(checkLoggedinThenReturnViewName(session, "redirect:/", "basicBoard"));
		...
		for (BoardDTO dto : boardlist) {
			if (dto.getBoard_contents() != null) {
				if (dto.getBoard_contents().contains("<img")) {
					includeImg.add(true);
				} else {
	...
	private String checkLoggedinThenReturnViewName(HttpSession session, String ifNotLoggedin, String ifLoggedin) {
	...
	public ModelAndView writingForm(
			HttpSession session,
			@RequestParam(value="ti", required=false, defaultValue="") String ti, 
