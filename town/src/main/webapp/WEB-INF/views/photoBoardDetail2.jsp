...
//----------------------------------------- page 버튼 -----------------------------------------------------------------
	...
	//대댓글에 사진첨부 ---------------------------------------------------------------------------------------------------------------------------------
	$(".recomment_file").change(function() {
        const formData = new FormData();
        ...
        $.ajax({
            ...
            success: function (data) {
                data.uploadPath = data.uploadPath.replace(/\\/g, '/');
                let imgUrl = "/display?fileName=" + data.uploadPath + "/" + data.uuid + "_" + data.fileName;
                $(this).parents().siblings(".recomment_img_preview").attr("src", imgUrl);
                $(this).parents().siblings(".recomment_img_preview").css("display","block");
            },
            error: function(request,status,error) {
			      		alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
			      		console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
					...
					<c:if test="${dto.parent_id != 0}">
						...
						<div class="recomment_upper">
							...
							<div class="recomment_profile">
								&nbsp;&nbsp;└&nbsp;&nbsp;&nbsp; 
								<div class="recomment_profileImg"><img src="${commentWriterProfileMap[dto.comment_writer] }"></div>
